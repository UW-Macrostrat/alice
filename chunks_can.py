...
def find_groups(year):
  ...
  cur.execute(""" 
    ...
    WHERE platea IN 
      (SELECT DISTINCT platea FROM distance_azimuth_matrix WHERE year > 500) AND year = %s
    ORDER BY platea ASC
  ...
  for plate in plates:
    ...
    if found < 1:
      ...
      cur.execute("""
        ...
        AND year = %s AND distance <= 30 AND platea = %s
  ...
  cur.execute("""
    DROP TABLE IF EXISTS chunks_can.reconstructed_%(year)s;
    CREATE TABLE chunks_can.reconstructed_%(year)s (
  ...
  for index, group in enumerate(groups):
    ...
    cur.execute("""
      INSERT INTO chunk_matrix_can (plateid, year, plates)
    ...
    cur.execute("""
      ...
      FROM (
        ...
        AND plateid IN (
          SELECT unnest(plates) FROM chunk_matrix_can WHERE plateid = %(plateid)s AND year = %(year)s
        )
      ) q;
    ...
    if num_records[0][0] > 1:
      ...
      cur.execute("""
        INSERT INTO chunks_can.reconstructed_%(year)s (plateid, geom) (
