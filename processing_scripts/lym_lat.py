...
class Task(object):
  ...
  def __call__(self, connection=None):
    ...
    pyCursor1.execute("""
        INSERT INTO plate_length_year_can_lat (year, plateid, coord, the_length)
        SELECT %(year)s AS year, plateid, coord, ST_Length_Spheroid(
        	ST_Intersection(
        		ST_SetSRID(half_degree_lat.geom, 4326),
        	 	r.geom
        	 )
        	 , 'SPHEROID["GRS_1980",6378137,298.257222101]')/1000 the_length
        FROM merge.reconstructed_""" + str(self.year) + """_merged r, half_degree_lat
        WHERE plateid IN (
